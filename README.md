...
This Node.js script automates the process of running a Postman collection for KYB (Know Your Business) verification against a list of businesses in a CSV file. It uses Newman to execute the API requests, then parses the JSON responses to generate a clean, consolidated CSV report.
...
* **Debugging Support**: Generates an intermediate JSON file containing the raw API responses for easy inspection and debugging.
...
You will also need:
1.  A **Postman Collection** file, exported as a `.json` file.
2.  An **Input Data** file in `.csv` format.
...
4.  In the "Export Collection" dialog, choose the recommended format (e.g., `Collection v2.1`).
...
* `NEWMAN_ITERATIONS`: **Crucially**, set this number to match the exact number of data rows in your `DATA_FILE` that you want to process.
